  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">留学国家</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">海外名校 PS/SOP 招生官审核与避坑指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 留学申请与个人陈述PS专家');
...
const presetOptions = [
  "英美名校计算机/数据科学 PS 个人陈述",
  "商学院金融/金工/MBA 目的陈述 SOP",
  "人文社科/传媒学术推荐信 RL 框架",
  "低 GPA 逆袭名校补充说明与解释信"
];
...
const metricsList = [
  {
    "key": "personalStatementAuthenticity",
    "label": "个人陈述真实真挚度"
  },
  {
    "key": "academicGoalClarity",
    "label": "学术目标清晰度"
  },
  {
  ...
  {
    "key": "languageNativeFluency",
    "label": "地道母语化表达度"
  },
  {
    "key": "admissionCompetitiveness",
    "label": "名校录取竞争力"
  }
];
const guideRules = [
  {
    ...
    "advice": "用具体的科研实验、代码行数或数据成果展示能力，而不是空洞夸赞自己聪明",
    "avoid": "严禁使用无依据的自我夸大词汇"
  },
  {
    "title": "Why School 契合度",
    "advice": "深入阅读目标系所官网，提及具体的教授实验室、特色课程或研究中心",
    "avoid": "切忌写通用的套话套用在所有学校上"
  },
  {
    "title": "逻辑主线贯穿",
    "advice": "全文围绕“我为何爱这个领域-我做了什么准备-为何选择贵校-我未来要做什么”主线",
    "avoid": "避免将 PS 写成 CV 的流水账扩写"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LIUXUE_SCORES\][\s\S]*?\[\/LIUXUE_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LIUXUE_SCORES\](.*?)\[\/LIUXUE_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('liuxue_history_records');
...
const saveHistory = () => {
  localStorage.setItem('liuxue_history_records', JSON.stringify(historyList.value));
};
